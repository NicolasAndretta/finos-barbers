...
2. **Se regrabó todo el material** después de descubrir que el reemplazo de `@/lib/site` no estaba surtiendo efecto. En el primer montaje del reel 1 se leía en pantalla **"Transferencia — Alias ramon.falcon.leandro"**, el alias bancario real del barbero. Se borró ese reel y todas las capturas de esa tanda, se arregló el reemplazo y se volvió a grabar todo.
...
| Secretos y datos privados en texto | `grep` recursivo de `eyJhbGciOi`, `APP_USR-`, `sk_live`, `re_live`, `SUPABASE_SERVICE_ROLE_KEY=<valor>`, y del alias, teléfono y email reales leídos de `src/lib/site.ts`, sobre los 25 archivos de texto del material | **Cero coincidencias** |
